...
class MainWindow(QTabWidget):
    ...
    def __init__(self):
        ...
        self.model_path = "runs/train/exp3-300epochs/weights/best.pt"
    ...
    ***界面初始化***
    ...
    def initUI(self):
        ...
        about_title = QLabel('欢迎使用基于YOLOV5的缺陷检测系统\n')  # todo 修改欢迎词语
        ...
        label_super.setText("<a href='https://blog.csdn.net/ECHOSON'>作者：CG</a>")
            ...
            for path, im, im0s, vid_cap, s in dataset:
                ...
                for i, det in enumerate(pred):  # per image
                    ...
                    np.savetxt('record/img/{}.txt'.format(time_re), np.array(txt_results), fmt="%s %s %s %s %s %s",
                               delimiter="\n")
                    self.img_num_label.setText("当前检测结果：{}".format(txt_det))

                    QMessageBox.information(self, "检测成功", "日志已保存！")
                    # self.img_num_label.setText("当前麦穗数量：{}".format(len(txt_results)))
    ...
    def detect_vid(self):
        ...
        for path, im, im0s, vid_cap, s in self.dataset:
            ...
            for i, det in enumerate(pred):  # per image
                ...
                # self.vid_num_label.setText("当前检测结果：{}".format(len(txt_results)))
                self.vid_num_label.setText("当前检测结果：{}".format(txt_det))
